...
const UserExpandedComponent = ({ data }) => {
    ...
    const handleChangeBirthday = e => {
        if (/^\d*$/.test(e.target.value) && e.target.value.length <= 6) setBirthday(e.target.value)
    }
    ...
    >
        <Row>
            ...
            <Form.Group as={Col} controlId="formGridName" sm={4}>
                ...
                <Form.Control type="text" placeholder="이름" value={username} onChange={handleChangeUsername} />
            </Form.Group>
            <Form.Group as={Col} controlId="formGridAge" sm={4}>
                <Form.Label>나이</Form.Label>
                <Form.Control type="text" placeholder="나이" value={data.age} readOnly />
            </Form.Group>
            <Form.Group as={Col} controlId="formGridGender" sm={2}>
                <p>성별</p>
                <Form.Check inline label={"남"} type={"radio"} checked={gender === 1} onChange={handleGenderSelectMale}/>
                <Form.Check inline label={"여"} type={"radio"}  checked={gender === 2} onChange={handleGenderSelectFemale}/>
            </Form.Group>
            <Col sm={2}>
                <p>등급 설정</p>
                <DropdownButton title={getGradeTitle(grade)} variant={getGradeVariant(grade)} onSelect={handleOnChangeGrade}>
                    <Dropdown.Item eventKey={0} ><Badge bg={getGradeVariant(0)} >{" "}</Badge>회원</Dropdown.Item>
                    <Dropdown.Item eventKey={10} ><Badge bg={getGradeVariant(10)} >{" "}</Badge>원장</Dropdown.Item>
                    <Dropdown.Item eventKey={15}><Badge bg={getGradeVariant(15)} >{" "}</Badge>매니저</Dropdown.Item>
                    <Dropdown.Item eventKey={20} ><Badge bg={getGradeVariant(20)} >{" "}</Badge>관리</Dropdown.Item>
                </DropdownButton>
            </Col>
        ...
        <Row>
            <Form.Group as={Col} controlId="formGridBirth" md={4}>
                ...
                <Form.Control type="text" placeholder="생년월일" value={birthday} onChange={handleChangeBirthday}/>
            </Form.Group>
            <Form.Group as={Col} controlId="formGridNum" md={4}>
                <Form.Label>전화번호</Form.Label>
                <Form.Control type="text" placeholder="전화번호" value={tel} className={data.valid ? "bg-success text-white" : "bg-danger text-white"} onChange={e => setTel(e.target.value)} readOnly={Boolean(auth)}/>
            </Form.Group>
            <Col md={2}>
                <span style={{display: "block"}}>권한 인증</span>
                <Button variant="info text-white" onClick={handleAuthCheck}>QR 인증</Button>
            </Col>
            <Col md={2}>
                <span style={{display: "block"}}>저장</span>
                <Button variant={save_message === "저장" ? "primary" : "info text-white"} onClick={handleSave}>{save_message}</Button>
            </Col>
        </Row>
        <hr />
