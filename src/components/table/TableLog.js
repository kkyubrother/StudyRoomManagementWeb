...
const CustomRowDataComponent = ({name, value, extra}) => (<Row><Col><span>{name}: </span><span>{value}</span>{extra}</Col></Row>)
const ExtraDataTypeTg = ({request, result, is_success}) => (<Row style={{backgroundColor: (is_success?"#2980b9":"#c0392b"), color:"whitesmoke"}}><Col><span>요청: </span><span>{request}</span></Col><Col><span>응답: </span><span>{result}</span></Col></Row>)
...
const ExpandedComponent = ({data}) => {
    ...
    return (
        <Container>
            ...
            <CustomRowDataComponent name={"이름"} value={data.username} extra={(data.sms === 1 ? <BsFillBookmarkCheckFill style={{color: "#23ab34"}} />: null)} />
            ...
            <CustomRowDataComponent name={"기록일시"} value={data.created.replace("T", " ")} />

            {row.log_type === "tg" && <ExtraDataTypeTg request={extra.update_text} result={extra.result_text} is_success={extra.success} />}
            {row.log_type === "error" && <Row><Col><Form.Control as="textarea" style={{height: 150}} value={JSON.stringify(extra, null, 2)}readOnly /></Col></Row>}
