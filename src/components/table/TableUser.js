...
const ModalBulkMessage = ({ show, onHide, users }) => {
  ...
  const listItems = users.map((u) => {
    let variant = "light";
    if (result[u.user_id] === "rejected") {
      variant = "danger";
    }
    if (result[u.user_id] === "fulfilled") {
      variant = "success";
    }
    return (
        ...
        <Row>
          ...
          <Col>
            <Form>
              <Form.Group
                className="mb-3"
                controlId="ModalBulkMessage.Form.ControlText"
              >
...
const TableUser = (props) => {
  ...
  useEffect(() => {
    if (/^\d+$/.test(filterText)) {
      ...
      switch (filterText) {
        case "등급:회원":
          setFilteredItems(
            rows.filter((item) => item.username && Number(item.grade) === 0),
          );
          break;
        case "등급:특별회원":
          setFilteredItems(
            rows.filter((item) => item.username && Number(item.grade) === 10),
          );
          break;
        case "등급:매니저":
          setFilteredItems(
            rows.filter((item) => item.username && Number(item.grade) === 15),
          );
          break;
        case "등급:관리자":
          setFilteredItems(
            rows.filter((item) => item.username && Number(item.grade) === 20),
          );
          break;
        default:
          setFilteredItems([]);
  ...
  const contextActions = useMemo(() => {
    const handleGrade = (grade) => {
      if (
        window.confirm(
          `진짜로 등급을 변경하시겠습니까?:\r ${selectedRows.map(
            (r) => r.username,
          )}?`,
        )
      ) {
