...
const TimeSelectorComponent = (
    ...
    return <Form.Group as={Row} className="mb-3" controlId="modalFormDT">
        <Form.Label column sm={2}>일시</Form.Label>
        <Col sm={10}>
            <InputGroup>
...
const CustomTimeDropdown = ({title = "시간 선택", start_time, end_time, onSelect}) => {
    // const value = moment().format("hh:mm");
    const items = [];

    const start_value = Math.floor((start_time.hour() * 60 + start_time.minute()) / 30)
    ...
    for (i=start_value; i <= end_value; i++) {
        const hour = (i - (i % 2)) / 2
        const minute = (i % 2) * 30
        items.push(<Dropdown.Item onClick={() => onSelect(hour, minute)} >{hour}:{minute === 0? "00": "30"}</Dropdown.Item>)
    }
