...
const DepartmentDropdownSelectorComponent = ({region = REGION_INITIAL_STATE, setRegion, regionTypePreset = "지역 분류", disabled = false}) => {
    ...
    useEffect(() => {
        getDepartments()
            .then(result => {
                ...
                setRegionEtc(result.map(value => <Dropdown.Item key={value.key} onClick={() => setRegion(value.key)}>{value.name}</Dropdown.Item>));
            })
    }, [])

    return (<Stack direction="horizontal" gap={3}><DropdownButton
            variant="outline-primary"
            title={(REGION_TYPE[regionType])?REGION_TYPE[regionType].name:regionType}
            id="input-group-dropdown-regionType"
            disabled={disabled}
        >
        {<Dropdown.Item key={REGION_TYPE.etc.name} onClick={() => handleOnClickRegionType(REGION_TYPE.etc.key)}>{REGION_TYPE.etc.name}</Dropdown.Item>}
        </DropdownButton>
        <DropdownButton
