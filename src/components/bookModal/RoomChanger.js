...
const RoomChangerComponent = ({book_id, room, handleChangeRoom, book_date}) => {
                ...
                <InputGroup>
                    ...
                    >
                        {rooms
                            ...
                            .map(value => <Dropdown.Item disabled={!value.available} key={`${value.type}-${value.no}`} onClick={() => handleSelectRoom(value.type, value.no)}>{value.name}</Dropdown.Item>)
                        }
                    </DropdownButton>
                    <Alert sm={3} variant={(result_good? "success" : "danger")}>{result}</Alert>
                </InputGroup>
            </Col>
        </Form.Group>
    )
