...
const InputPeopleNoV2 = ({people_no, setPeopleNo, disabled=false}) => {
    ...
    return <Form.Group
        ...
        <Form.Label column sm="2">{"모임"}</Form.Label>
        <Col sm="10">
            <DropdownButton title={getPeopleNoTitle(people_no)} disabled={disabled}>
                <Dropdown.Item key={-2} onClick={() => setPeopleNo(-2)}> 팀 </Dropdown.Item>
                <Dropdown.Item key={-4} onClick={() => setPeopleNo(-4)}>개인</Dropdown.Item>
            </DropdownButton>
        </Col>
    </Form.Group>
