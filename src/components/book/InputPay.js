...
const InputPay = ({
  ...
  setEtcReason,
  disabled = false,
  options = ["card", "transfer", "saved_money.d", "saved_money.p", "etc"],
  setPaid = null,
}) => {
  ...
  return (
    <Form.Group as={Row} controlId="modalFormPay">
        ...
        <InputGroup>
          ...
          >
            {options.includes("card") && (
              <Dropdown.Item key={"card"} onClick={() => setPayType("card")}>
                <BsFillCreditCard2BackFill color={"#138d75"} />
                카드
              </Dropdown.Item>
            )}
            ...
            {options.includes("notion") && (
              <Dropdown.Item
                key={"notion"}
                onClick={() => setPayType("notion")}
              >
                노션
              </Dropdown.Item>
            )}
            {options.includes("etc") && (
              <Dropdown.Item key={"etc"} onClick={() => setPayType("etc")}>
                기타
              </Dropdown.Item>
            )}
          </DropdownButton>
