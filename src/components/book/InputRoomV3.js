    ...
    return <Col sm="3">
        ...
        >
            {rooms
                ...
                .map(
                    value => <Dropdown.Item
                        disabled={!value.available} key={`${value.type}-${value.no}`}
...
const InputRoomV3 = ({room, setRoom, date, start_time, end_time, disabled=false}) => {
    ...
    return <Form.Group as={Row} controlId="modalFormRoomDropdown">
        <Form.Label column sm="2">장소</Form.Label>
        <Form.Label column sm="2">스터디룸</Form.Label>
        <DropdownRoom
            title="스터디룸"
