...
const LockerFormComponent = (props) => {
  ...
  const selectedUser =
    users.filter((user) => user.user_id === data.user_id).length === 0
      ? {}
      : users.filter((user) => user.user_id === data.user_id)[0];
  ...
  const lockerItems = lockers
    ...
    .map((item) => (
      <option
        ...
        selected={item.locker_id === data.locker_id}
        disabled={isDisable(item.locker_id)}
      >
        {lockerItemText(item)}
      </option>
    ));
  ...
  useEffect(() => {
    ...
    if (inputUserFind === "") {
      ...
      const searchedUsers = users.filter(
        (value) => value.num === inputUserFind,
      );
      if (searchedUsers.length === 0) return;

      setData({
        ...data,
        user_id: searchedUsers[0].user_id,
  ...
  return (
    <Form>
      <Form.Group className="mb-3" controlId="formLockerSelect">
        ...
        <Form.Select
          onChange={(e) =>
            setData({ ...data, locker_id: Number(e.target.value) })
          }
      ...
      <Form.Group className="mb-3" controlId="formUserId">
        <Form.Label>
          ...
          {selectedUser.username &&
            `(선택: [${selectedUser.user_id}] ${selectedUser.username})`}
        </Form.Label>
        <Form.Control
          type="text"
          placeholder="사용자 ID 또는 전화번호를 입력하시오."
          onChange={(e) => setInputUserFind(e.target.value)}
          disabled={defaultData}
          value={inputUserFind}
        />
      ...
      <Form.Group className="mb-3" controlId="formRentalPeriod">
        ...
        <Form.Select
          aria-label="Locker Select"
          disabled={defaultData}
          onChange={(e) =>
            setData({ ...data, rental_period: Number(e.target.value) })
          }
        >
          <option value="1" selected={data.rental_period === 1}>
            1개월
          </option>
          <option value="2" selected={data.rental_period === 2}>
            2개월
          </option>
          <option value="3" selected={data.rental_period === 3}>
            3개월
          </option>
          <option value="4" selected={data.rental_period === 4}>
            4개월
          </option>
          <option value="5" selected={data.rental_period === 5}>
            5개월
          </option>
          <option value="6" selected={data.rental_period === 6}>
            6개월
          </option>
        </Form.Select>
      </Form.Group>
      ...
      <Form.Group className="mb-3" controlId="formPaymentRequired">
        <Form.Check
          type="checkbox"
          label="기한이 끝난후 비용을 내야합니다."
          onChange={() =>
            setData({ ...data, payment_required: !data.payment_required })
          }
