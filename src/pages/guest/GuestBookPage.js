...
import {PAY_STATUS_CONFIRM, PAY_STATUS_REJECT, PAY_STATUS_WAITING, postPay} from "../../api/pays";
...
const ModalWaitPay = ({show, handleClose, title, message}) => (
    ...
    >
        ...
        <Modal.Body>
            {message?message : (<><Spinner animation={"border"} variant={"primary"} />예약중입니다...</>)}
        </Modal.Body>
        <Modal.Footer>
...
const GuestBookPage = props => {
    ...
    console.debug(`GuestBookPage::location::${JSON.stringify(location, null, 2)}`)
    ...
    const options = isStaff ? ["card", "saved_money.d", "transfer", "notion", "etc"] : ["card", "saved_money.d"]
    ...
    const handleOnSelectStartTime = (hour, minute) => {
        ...
        if (guest_state.end_time && t.isSameOrAfter(guest_state.end_time)) alert("올바르지 않은 시간입니다.");
        else changeStartTime(dispatch, t);
    }
    const handleOnSelectEndTime = (hour, minute) => {
        const t = moment(`${hour}:${minute}`, "HH:mm")
        if (guest_state.start_time && t.isSameOrBefore(guest_state.start_time)) alert("올바르지 않은 시간입니다.");
    ...
    useEffect(() => {
        ...
        if (d.isoWeekday() >= 6) {
            ...
            getConfigBookWeekendClose()
                .then(value => setMaxTime(moment(value, "hh:mm:ss")))
        }
        else {
            getConfigBookWeekdaysOpen()
                .then(value => {
    ...
    useEffect(() => {
        if (guest_state.start_time && guest_state.end_time && guest_state.room)
            setPaid(calcPaid(guest_state.room.type, guest_state.start_time, guest_state.end_time))
    }, [guest_state])
    useEffect(() => {
    ...
    const handleOnClickBook = () => {
        if (!guest_state) {alert("비정상 접근")}
        else if (!guest_state.username) {alert("QR 비인증 접근")}
        else if (!guest_state.start_time || !guest_state.end_time) {alert("시간 설정 필요")}
        else if (!guest_state.room) {alert("방 설정 필요")}
        else if (!guest_state.department) {alert("지역 설정 필요")}
        else if (!guest_state.people_no) {alert("인원 설정 필요")}
        else if (!guest_state.pay_type) {alert("결제 정보 설정 필요")}
        else {
            ...
            .then(result => {
                console.debug(`GuestBookPage::handleOnClickBook::postBook::result::${JSON.stringify(result, null, 2)}`)
        if (is_active_paying) return
                ...
                .then(pay => {
                    console.log(`GuestBookPage::handleOnClickBook::postBook::postPay::pay::${JSON.stringify(pay, null, 2)}`)
                    ...
                    const timer = 60;
                    const callback = data => {
                        if (data.status === PAY_STATUS_WAITING) setModal(<ModalWaitPay show={true} title={data.title} message={data.message} />)
                        else if (data.status === PAY_STATUS_CONFIRM) {
                            setModal(<ModalWaitPay show={true} title={data.title} message={data.message}
                                                   handleClose={() => isStaff ? navigate("/books") : navigate("/")}/>);
                    ...
                    setTimeout(WaitTransaction, 500, ...[pay_id, timer, callback, user_id, book_id])
                })
                    .catch(error => {
                        console.error(error);
                        deleteBook(location.state.user.user_id, result.book_id, `Pay.Error`)
                            .then(res => console.log(res));
                        setModal(<ModalBooked
                            show={true}
                            title={"오류 발생"}
                ...
                .catch(reason => {
                    ...
                    setModal(<ModalBooked
                        show={true}
                        title={"오류 발생"}
                        message={reason?reason.response.data.message : "다시 신청바랍니다."}
                        handleClose={() => setModal(null)}
                    />)
                })
        }
    ...
    >
        <AsyncImage style={styles.image} src={`/api/books/${guest_state.book_date}.png?_=${(book_date).valueOf()}`} />
        <InputNameV2 value={guest_state.username} />
        {
            isStaff
            ? (guest_state.book_date ? <InputDateV3 value={guest_state.book_date} setValue={v => handleOnSelectDate(v.getFullYear(), v.getMonth() + 1, v.getDate())} /> : "loading")
        ...
        <Row>
            <Button
                as={Col}
                onClick={() => navigate(window.location.pathname === '/staff/book' ? "/books" : "/")}
