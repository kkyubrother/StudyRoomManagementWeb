...
import WaitTransaction, {isPayConfirm} from "../../controller/WaitTransactionController";
...
const getTitleFromBook = (book) => (
    `[${book.department}] ${book.user.username} ${book.people_no} 명  ${book.start_time.format("HH:mm")}~${book.end_time.format("HH:mm")}`
)
...
const GuestBookInfoComponent = ({book, pay_type, setPayType, etc_reason, setEtcReason, disabled}) => {
...
const BookItem = ({book, handleSelectBook, selected_book}) => {
    const is_selected = book && selected_book && selected_book.book_id === book.book_id;
    const is_paid = book && book.pay && book.pay.status === PAY_STATUS_CONFIRM;

    const style = is_selected? {...styles.colBooksActive}:(is_paid? {...styles.colBooksPaid}:{...styles.colBooks});

    return <Col
        sm
        style={style}
        onClick={book ? () => handleSelectBook(book) : null}
    >
...
const GuestBookCheckPage = props => {
    // console.log(`GuestBookCheckPage::props::${JSON.stringify(props, null, 2)}`);
    ...
    useEffect(() => {
        if (!location.state || !location.state.user || !location.state.user.user_id) navigate('/');
        const today_moment = moment();
        getBooksBy(location.state.user.user_id)
            .then(results => {
                setBooks(results.map(value => {
                    return {
                        ...value,
                        start_time: moment(value.start_time, "hh:mm:ss"),
                        end_time: moment(value.end_time, "hh:mm:ss"),
                    }
                }).filter(value => moment(value.book_date).isSame(today_moment, "day")));
            })
    }, [location.state, navigate])

    const handleSelectBook = book => {
        // console.debug(`handleSelectBook::book::${JSON.stringify(book, null, 2)}`);
    ...
    const setModalPayError = error => {
        setModal(<ModalWaitPay show={true} title={"오류 발생"} message={error ? error.response.message : "다시 결제바랍니다."}
    ...
    const handleOnClickPay = () => {
        ...
        const pays_pay_type = pay_type + (book.department ? "." + book.department : "");
        ...
        postPay(user_id, book_id, cashier, pays_pay_type, paid, etc_reason, client_name)
            .then(pay => {
                const pay_id = pay.pay_id;
                const timer = 60;
                const callback = data => {
                    if (data.status === PAY_STATUS_WAITING) setModal(<ModalWaitPay show={true} title={data.title}
    ...
    const handleOnClickCancel = () => {
        ...
        cancelBookControl(book_id, qr_code, cashier,
            (m, s) => {
                setMessage(m);
                setShowSpinner(s)
            },
    ...
    >
        ...
        <Container>
            {books.map((value, index) => {
                if ((index * 3) >= books.length) return null
                    ...
                    return (<Row style={styles.rowBooks}>
                        <BookItem book={books[(index * 3)]} handleSelectBook={handleSelectBook} selected_book={book}/>
                        <BookItem book={books[(index * 3 + 1)]} handleSelectBook={handleSelectBook} selected_book={book}/>
                        <BookItem book={books[(index * 3 + 2)]} handleSelectBook={handleSelectBook} selected_book={book}/>
                    </Row>)
                }
            })}
        </Container>
        ...
        {message && <Modal show={true}>
            <Modal.Body>{show_spinner && <Spinner animation={"border"}/>}{message}</Modal.Body>
            <Modal.Footer>{!show_spinner && <Button onClick={() => setMessage(null)}>확인</Button>}</Modal.Footer>
        </Modal>}
    </Container>
}
