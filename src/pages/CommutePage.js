...
const CommuteAuthComponent = (props) => {
  ...
  const updateUserCommuteState = (users) => {
    Promise.allSettled(
      users.map(async (v) => {
        return await getCommute("person", today, v.user_id);
      }),
    )
      .then((result) => {
        return result.map((v, i) => {
          ...
          else if (v.value.data.length === 0)
            user.status = UserStatus.NOT_ENTER;
          else {
            const last_commute = v.value.data.pop();
            user.status = last_commute.exit_time
              ? UserStatus.EXITED
              : UserStatus.NOT_EXIT;

            user.enter_time = last_commute.enter_time
              ? moment(last_commute.enter_time).subtract(9, "hour")
              : null;
            user.exit_time = last_commute.exit_time
              ? moment(last_commute.exit_time).subtract(9, "hour")
              : null;
          }
          return user;
        });
  ...
  const handleName = async (user) => {
    ...
    const message = `${user.username}님, ${
      user.status === UserStatus.NOT_EXIT ? "퇴근" : "출근"
    }하시나요?`;
  ...
  const convertBadge = (user) => {
    switch (user.status) {
      case "error":
        return <Badge bg="danger">에러</Badge>;
      case "not_enter":
        return <Badge bg="secondary">모름</Badge>;
      case "not_exit":
        return <Badge bg="primary">출근</Badge>;
      case "exited":
        return <Badge bg="success">퇴근</Badge>;
      default:
        return null;
    }
  };
  ...
  return (
    <Col>
      <Row>
        <h1 className="text-center">근태 관리</h1>
      </Row>
      <Table>
        <thead>
          <th>이름</th>
            ...
            users.map((user) => {
              ...
              return (
                <tr>
                  ...
                  <td>
                    {user.enter_time ? user.enter_time.format("HH:mm") : "-"}
                  </td>
                  <td>
                    {user.exit_time ? user.exit_time.format("HH:mm") : "-"}
                  </td>
                </tr>
              );
            })
...
const CommuteTableComponent = (props) => {
  ...
  const convertResult = (res) => {
    return {
      commute_id: res.commute_id,
      enter_time: moment(res.enter_time),
      exit_time: moment(res.exit_time),
      user: res.user,
      record: res.record,
    };
  };
      ...
      <Row>
        ...
        <Col md={4}>
          <Button
            onClick={() => {
              setLoading(true);
              setMonthOrDate(!monthOrDate);
            }}
            disabled={loading}
          >
            {monthOrDate ? "일별로 변경" : "월별로 변경"}
          </Button>
        </Col>
      </Row>
      <Table>
            ...
            result.map((res) => {
              return (
                <tr>
                  ...
                  {monthOrDate && (
                    <td>
                      {res.enter_time.subtract(9, "hour").format("YYYY-MM-DD")}
                    </td>
                  )}
                  <td>{res.enter_time.subtract(9, "hour").format("HH:mm")}</td>
                  <td>
                    {res.exit_time.isValid()
                      ? res.exit_time.subtract(9, "hour").format("HH:mm")
                      : "-"}
                  </td>
                  <td>{res.record}</td>
                </tr>
