...
function AdminLogin() {
    ...
    const [client, setClient] = useState({name: "경호", client: "guard"});
    ...
    useEffect(() => {
        // for debug
      const timer_debug = window.setTimeout(async () => setOtp(await get_otp_code_test()), 0)
    ...
    return (
        <div><Container
            ...
            <img src={icon_main} alt="icon_main" style={{width: 72, height: 72, marginTop: 150}} />
            <h1>로그인이 필요합니다</h1>
            <div className="hr" />
            <Form>
                <Form.Group mb={4} controlId="formOTP">
                    ...
                    <InputGroup>
                        <DropdownButton title={client.name} variant="outline-secondary" id="admin-login-input-client-name" >
                            <Dropdown.Item onClick={() => setClient({name: "관리", client: "admin"})}>관리</Dropdown.Item>
                        </DropdownButton>
                        <Form.Control onChange={handleFormOtpOnChange} variant="outline-secondary" type="number" placeholder="OTP" value={otp} pattern="[0-9]*" />
                  </InputGroup>
            ...
            <Col>
                <Button variant="secondary" size="lg" onClick={handleOnClickHome}>({timer}초 후)홈으로</Button>
                <Button variant="primary" size="lg" onClick={handleOnClick}>확인</Button>
            </Col>
