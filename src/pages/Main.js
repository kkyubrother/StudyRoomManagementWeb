...
const Main = props => {
    ...
    const [show, setShow] = useState(Boolean(location.state && location.state.message));
    ...
    return <Container
        ...
        fluid
    >
        <Row style={styles.rowAdmin} onClick={() => navigate("/admin/login", {state:{backref: "/books"}})}>
            <h1 style={{textAlign: "center", fontWeight: "bold"}}>관리자 진입</h1>{" "}</Row>
        <Row style={styles.row} >
            <Col style={styles.col} >
                <CardItem
                    img_src={"img_guest/icon-book.png"}
                    text={"현장 사용"}
                    onClick={() => navigate("/guest", {state:{backref: "/guest/book"}})}
                />
            </Col>
            {/*<Col style={styles.col} >*/}
            {/*    <CardItem*/}
            ...
            <Col style={styles.col} >
                <CardItem
                    ...
                    text={"예약 확인"}
                    onClick={() => navigate("/guest", {state:{backref: "/guest/book_check"}})}
                />
            </Col>
        </Row>
        <Row style={styles.rowFooter}>
            <Col style={styles.colFooter}>{server_title}({server_version}){!server_alive && <MdReportProblem style={{color: "#B82647"}} />}</Col>
            <Col style={styles.colFooter}>{copyright}</Col>
            <Col style={styles.colFooter}>{document.querySelector('meta[name="description"]').content}</Col>
        </Row>
        <ModalMessage show={show} onHide={() => setShow(false)} message={location.state && location.state.message} />
    </Container>
}
