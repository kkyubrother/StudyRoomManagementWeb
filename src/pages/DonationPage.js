...
const DonationPage = props => {
    console.log(`DonationPage::props::${JSON.stringify(props, null, 2)}`)
                    ...
                    getPay(pay_id)
                        .then(pay_result => {
                            if (pay_result.status.startsWith(PAY_STATUS_CONFIRM)) {
                                ...
                                setModal(<ModalWaitPay show={true} title={"결제 완료"} message={"결제되었습니다."} handleClose={() => navigate("/books")} />)
                            }
                            else if (pay_result.status.startsWith(PAY_STATUS_REJECT)) {
                                setModal(<ModalWaitPay show={true} title={"결제 실패"} message={pay_result.comment?pay_result.comment:"다시 결제바랍니다."} handleClose={() => navigate("/books")} />)
                            }
                        ...
                        .catch(error => {
                            console.error(error);
                            setModal(<ModalWaitPay show={true} title={"결제 실패"} message={error?error.response.message:"다시 결제바랍니다."} handleClose={() => navigate("/books")} />)
                        })
                } else {
                    setModal(<ModalWaitPay show={true} title={"결제 대기 시간 초과"} message={"다시 결제바랍니다."} handleClose={() => navigate("/books")} />)
                }
            }
