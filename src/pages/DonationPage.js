...
const DonationPage = (props) => {
  ...
  const handleOnClickDonation = () => {
    ...
    postPayDonation(
      ...
      .then((pay) => {
        console.log(
          `GuestDonationPage::handleOnClickDonation::postPayDonation::pay::${JSON.stringify(
            pay,
            null,
            2,
          )}`,
        ...
        const wait_transaction = (pay_id, timer) => {
          console.log(
            `GuestDonationPage::handleOnClickDonation::postPayDonation::timer::${JSON.stringify(
              timer,
              null,
              2,
            )}`,
          ...
          setModal(
            <ModalWaitPay
              show={true}
              title={"결제 대기중"}
              message={`결제해주세요(${Math.round(timer / 2)} 초)`}
            />,
          );
          if (timer > 0) {
            getPay(pay_id)
              .then((pay_result) => {
                if (pay_result.status.startsWith(PAY_STATUS_CONFIRM)) {
                  console.log(
                    `GuestBookPage::handleOnClickBook::postBook::postPay::pay_result::${JSON.stringify(
                      pay_result,
                      null,
                      2,
                    )}`,
