...
const putRental = (locker_rental_id, locker_id, deadline, rental_key, payment_required, deposit) => {
    return axios.put(`/api/locker/${locker_rental_id}`, {deadline: deadline, locker_id, rental_key, payment_required, deposit})
}
...
const postRental = (user_id, locker_id, department, rental_period, deposit, id_picture, rental_key, payment_required, licenser_id) => {
...
const LockerPage = () => {
    ...
    const loadRental = () => {
        getRental(0)
            ...
            .then(result => {
                ...
                Promise.all(result.map(item => getLocker(item.locker_id).then(r => r.data)))
                    .then(locker_result => {
                        result.forEach(rental => {
                            rental.locker = locker_result.filter(locker => locker.locker_id === rental.locker_id)[0];
                        })
                    })
                    .then(() => {
                        Promise.all(result.map(item => getUser(sessionStorage.getItem("Authorization"), item.user_id)))
                            .then(user_result => {
                                result.forEach(rental => {
                                    rental.user = user_result.filter(user => user.user_id === rental.user_id)[0];
                                })
                                setRentals(result);
                            })
                    })
    ...
    const getKeyStatus = (key_status) => (key_status === 0) ? '보유' : ((key_status === 1) ? "대여" : (key_status === 2) ? "분실" : "알 수 없음")
    ...
    const onEditSave = (data) => {
        const deadlineText = `${deadlineValue}T${selectedRental.deadline.subtract(9, 'h').format('hh:mm:ss')}`;
        try {
            moment(deadlineText);
            putRental(selectedRental.locker_rental_id, data.locker_id, deadlineText, data.rental_key, data.payment_required, data.deposit)
                .then(() => loadRental())
                .catch(() => {
                    alert(`변경 실패`)
                })
    ...
    const onEditEnd = () => {
        if (!window.confirm(`${selectedRental.user.username}님의 대여를 종료하시겠습니까?`)) return;
    ...
    const LockerContainer = (location_group) => {
        ...
        let location_group_config = lockerConfig.location.filter(v => v.group_id === location_group);
        if (location_group_config.length === 0) {
            rentals.filter(v => v.locker.location_group === location_group).map(v => {
        ...
        for (let i = 0; i < y; i++) {
            const result_x = [];
            for (let j = 0; j < x; j++) {
                let rental = rentals.filter(v => v.locker.location_group === location_group && v.locker.location_x === (j + 1) && v.locker.location_y === (i + 1));
                ...
                if (rental.length) {
                    ...
                    const onClick = () => {setSelectedRental(rental); onEditValue(rental);}

                    const text = editMode ? `${locker_num}`.padStart(2, "0") : `[${locker_num}번][${rental.department}] ${rental.user.username}`;
                    const cardStyle = editMode ? {borderInlineWidth: "0.5rem"} : {}
                    r = (<Card bg={variant} text="white" onClick={onClick} border={selectedRental.locker_rental_id === rental.locker_rental_id ? "info" : ""} style={cardStyle}><Card.Body>
                        <Card.Title>{text}</Card.Title>
                    </Card.Body></Card>)
                }
                else {
                    const text = editMode ? `${locker_num}`.padStart(2, "0") : `[${locker_num}번]없음`;
                    r = (<Card bg="dark" text="white"><Card.Body>
                        <Card.Title>{text}</Card.Title>
                    </Card.Body></Card>)
                }

                const colStyle = editMode ? {paddingLeft: "1px", paddingRight: "1px"} : {}
                result_x.push(<Col style={colStyle}>{r}</Col>);
            }

            const rowStyle = editMode ? {marginTop: "1px", marginBottom: "1px"} : {marginTop: "5px", marginBottom: "5px"}
    ...
    return (<div>
        <Navi/>
        <Container>
            <Row style={{textAlign: "center"}}><h1>사물함 위치 그룹 <Dropdown className={"d-inline-block"} onSelect={e => setLocationGroupId(Number(e))}>
            ...
            {editMode ? null : <Container>
                {LockerContainer(locationGroupId)}
            </Container>}

            <Row><Col><div style={{width: 10, height: 10}}></div></Col></Row>
            <Row>
                {editMode && <Col style={{maxWidth: "300px"}}>
                    {LockerContainer(locationGroupId)}
                </Col>}
                <Col>
                    <Card>
                        <Card.Body>
                            ...
                            <Card.Text>대여 사용자: {`${selectedRental.user.username}(${selectedRental.user.num})`}</Card.Text>
                            <Card.Text>열쇠: {getKeyStatus(selectedRental.locker.main_key)}</Card.Text>
                            <Card.Text>대여시작일: {selectedRental.created.format('YYYY-MM-DD')}</Card.Text>
                            <Card.Text>대여마감일: {selectedRental.deadline.format('YYYY-MM-DD') + " 까지"}</Card.Text>
                            {editMode && (<InputGroup>
                                    <Form.Control value={deadlineValue} onChange={e => setDeadlineValue(e.target.value)} autoFocus={true} placeholder="YYYY-MM-DD" />
                                    <InputGroup.Text>으로 수정</InputGroup.Text>
                            </InputGroup>
                            )}
                            <Card.Text>대여: {selectedRental.vitalization ? "대여중" : "종료됨"}</Card.Text>
                            <Card.Text>비용: {selectedRental.payment_required ? "지불함" : "지불안함"}</Card.Text>
                    ...
                    </Card>
                    {
                        editMode
                            ? <>
                                <Button onClick={() => setEditMode(false)} variant="secondary">취소</Button>
                                <Button onClick={() => onEditEnd()} variant="danger">대여종료</Button>
                            </>
                ...
                <Col>
                    {
                        editMode
                            ? <LockerFormComponent users={users} rentals={rentals} lockers={lockers} handleSave={onEditSave}
                                                   defaultData={{
                            ...
                            : <LockerFormComponent users={users} rentals={rentals} lockers={lockers} handleSave={saveRental}/>
                    }
                </Col>
            </Row>
        </Container>
