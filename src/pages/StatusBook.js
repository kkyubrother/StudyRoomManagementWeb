...
import {Accordion, Offcanvas, ProgressBar} from "react-bootstrap";
...
const StatusBook = props => {
    ...
    const fetchData = async (selectDate, selectDateDuration) => {
        const dateArray =  [...(new Array(selectDateDuration))].map((_, i) => moment(selectDate).add(i, "day").format("YYYY-MM-DD"));
        ...
        newDataList.forEach(valueAtDate => {
            valueAtDate.filter(filterBook).forEach(v => {
                if (!v.room) {}
                    ...
                    const start_time_sec = Number(v.start_time.split(":")[0]) * 60 + Number(v.start_time.split(":")[1]);
                    let end_time_sec = Number(v.end_time.split(":")[0]) * 60 + Number(v.end_time.split(":")[1]);
                    if (end_time_sec === 1339) {end_time_sec = 1440}
                    rental.study_total_sec += (end_time_sec - start_time_sec);
                }
                else if (v.room.type === 2) {rental.seminar_total_count++}
                else if (v.room.type === 4) {rental.hall_total_count++}
            });
        });
        return rental;
    }
    useEffect(() => {
        (async () => {
            ...
            for (let i = 0; i <= date_due; i++) {
                ...
                if ((dueType === DUE_TYPE.MONTH ? date_in_for.date() === moment(date_in_for.format("yyyy-MM-DD")).endOf("month").date() : date_in_for.day() === SATURDAY) || date_in_for.isSame(end_date, "day")) {
                    const result = {
            ...
            for (const v of readyList) {
                ...
                result.push({status: "fulfilled", value: await fetchData(v.start_date, v.due)});
            }

            // const result = await Promise.allSettled(readyList.map(v => fetchData(v.start_date, v.due)))
            setBookStatus(result.map((v, i) => {
    ...
    const updateDetailData = (selectDate, selectDateDuration) => fetchDetailData(selectDate, selectDateDuration).then(setBookDetailStatus);
    const fetchDetailData = async (selectDate, selectDateDuration) => {
        ...
        return newDataList.map((valueAtDate, i) => {
            const detail = valueAtDate.filter(filterBook).map(v => {
                ...
                if (!v.room) {result.due_hour = "0"}
                else {
                    const start_time_minute = Number(v.start_time.split(":")[0]) * 60 + Number(v.start_time.split(":")[1]);
                    let end_time_minute = Number(v.end_time.split(":")[0]) * 60 + Number(v.end_time.split(":")[1]);
                    if (end_time_minute === 1339) {end_time_minute = 1440}
                    result.due_hour = ((end_time_minute - start_time_minute) / 60).toFixed(1)
    ...
    return <Container>
        ...
        <div className={"text-center d-flex flex-row"}>
            ...
            <div className="form-check form-check-inline">
                <input className="form-check-input" type="radio" name="sumDuration" id="sumDurationWeek"
                       value="week" checked={dueType === DUE_TYPE.WEEK} onChange={() => setDueType(DUE_TYPE.WEEK)}/>
                    <label className="form-check-label" htmlFor="sumDurationWeek">주간</label>
            </div>
            <div className="form-check form-check-inline">
                <input className="form-check-input" type="radio" name="sumDuration" id="sumDurationMonth"
                       value="month" checked={dueType === DUE_TYPE.MONTH} onChange={() => setDueType(DUE_TYPE.MONTH)}/>
                    <label className="form-check-label" htmlFor="sumDurationMonth">월간</label>
            </div>
                </span>
            ...
            <span className={"h1"}>~</span>
                ...
                dateFormat={dueType === DUE_TYPE.MONTH ? "yyyy-MM" : "yyyy-MM-dd"}
                selected={endDate}
                onChange={dueType === DUE_TYPE.MONTH ? d => setEndDate(moment(d).endOf("month").toDate()) : setEndDate}
                customInput={<CustomDatePickerInput />}
                locale={ko}
                showMonthYearPicker={dueType === DUE_TYPE.MONTH}
            />
        ...
        <Table bordered={true}>
            ...
            {bookStatus.map(v => <tr onClick={() => updateDetailData(v.start_date, v.due)}>
        ...
        {bookDetailStatus.map((data, eventKey) => {
            return <Accordion>
                <Accordion.Item eventKey={`${eventKey}`}>
                    <Accordion.Header>{data.date} ({data.detail.length}건)</Accordion.Header>
                    <Accordion.Body>
                        <Table>
                            <thead>
                            <tr>
