...
const Exports = () => {
  ...
  const items = type_list
    .map((value) => {
      if (is_manager && value.grade <= 15)
        return (
          <Dropdown.Item
            disabled={!value.available}
            onClick={() => setSelect(value.id)}
          >
            <Badge bg={value.tag.variant}>{value.tag.text}</Badge>
            {value.name}
          </Dropdown.Item>
        );
      else if (is_admin && value.grade <= 20)
        return (
          <Dropdown.Item
            disabled={!value.available}
            onClick={() => setSelect(value.id)}
    ...
    <div>
      ...
      <Container>
            ...
            <DropdownButton
              title={
                select ? (
                  <>
                    <Badge
                      bg={type_list.find((v) => v.id === select).tag.variant}
                    >
                      {type_list.find((v) => v.id === select).tag.text}
                    </Badge>
                    {type_list.find((v) => v.id === select).name}
                  </>
                ) : (
                  "선택하세요"
                )
        ...
        <Button
          ...
          disabled={
            !user || !type_list.find((v) => v.id === select) || downloading
          }
        >
          {downloading && <Spinner animation={"border"} />}다운로드
        </Button>
        {download_error && (
          <span className={"text-danger"}>{download_error}</span>
        )}
      </Container>
    </div>
  );
