...
const CustomDayHeader = props => {
    ...
    return <div className={className} style={{height: rowHeight, fontSize: "1.4rem"}}>{convertRoomToRoomName(day)}</div>
}
...
const TimetableItemWithText = ({event, defaultAttributes, classNames}) => {
    ...
    const text_third = moment(event.startTime).format("HH:mm") + " - " + moment(event.endTime).format("HH:mm");
    ...
    return (
            <div {...defaultAttributes}
                 style={{
                     ...
                     background: "#2b463c",
                     color: (event.data.pay && event.data.pay.status === "confirm") ? "#9ba9b4" : "#cbe541",
                     border: "solid",
                     fontSize: "1.2rem"
                 }}
                 title={event.name}
...
const Books = props => {
    ...
    const updateBookData = async () => {
        ...
        const books = await getBooksByBookDate(moment(book_date).format("YYYY-MM-DD"));
        loadBookData(books);
    }
    ...
    const handleOnClickNewBook = () => {
        navigate("/staff/book", {replace: true, state: {user: {"age":null,"birthday":null,"chat_id":-1001583498229,"created":"2022-01-31T23:54:44","delete_que":null,"department":null,"gender":null,"grade":20,"modified":"2022-01-31T23:54:44","num":null,"sms":1,"status":null,"tg_name":null,"user_id":1,"username":"웹사이트","valid":true}, qr_code: "100001xJemApdrPDbgLOy5t0IgdpzkU6xX7iDzBpmc8VRtAty7MhjTwEXy-xIp1C2gyPCeeFcXZSJIqeTkTJ3FqWPMRQ"}});
    }

    const handleOnClickDonation = user => {
        navigate("/donation", {replace: true, state: {user: user, qr_code: ""}});
    }
    ...
    return (<div>
        ...
        >
            ...
            <Modal.Footer><Button
                ...
                variant={"secondary"}>닫기</Button></Modal.Footer>
        </Modal>}
        {
