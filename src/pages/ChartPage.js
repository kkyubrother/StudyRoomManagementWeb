...
import {convertConfigHourToHour, convertRoomToRoomName} from "../util/convert";
...
import {classifyRoomBook, convertBookItem, getDefaultRoom} from "../util/book";
import DatePicker from "react-datepicker";
import {getSnackBarPaysByDate, getSnackBarProducts, getSnackBarStock} from "../api/snackbar";
...
const getNoShowData = async (date) => await axios.get("/api/logs", {params: {date, type: "book.del.record", need_all: "all"}}).then(response => response.data.logs);
const getUserCancelData = async (date) => await axios.get("/api/logs", {params: {date, type: "user.book.cancel", need_all: "all"}}).then(response => response.data.logs);
const getAdminCancelData = async (date) => await axios.get("/api/logs", {params: {date, type: "admin.book.delete", need_all: "all"}}).then(response => response.data.logs);
...
const RoomBookChartComponent = props => {
    ...
    const dateArray =  [...(new Array(selectDateDuration))].map((_, i) => moment(selectDate).subtract(i, "day").format("YYYY-MM-DD")).reverse();
    const updateBookData = async () => {
        setData((await Promise.allSettled(dateArray.map(async v => getBooksByBookDate(v)))).map(v => v.value));
    }
    ...
    const updateNoShowData = async () => {
        setNoShowData((await Promise.allSettled(dateArray.map(async v => getNoShowData(v)))).map(v => v.value));
    }
    const updateUserCancelData = async () => {
        setUserCancelData((await Promise.allSettled(dateArray.map(async v => getUserCancelData(v)))).map(v => v.value));
    }
    const updateAdminCancelData = async () => {
        setAdminCancelData((await Promise.allSettled(dateArray.map(async v => getAdminCancelData(v)))).map(v => v.value));
    }

    const blocked_room_book = data.map(v => v.filter(v => v.status === 400).length);
    const success_room_book_success = data.map((v, i) => v.filter(v => v.pay && v.pay.status === "confirm").length - blocked_room_book[i]);
    const fail_room_book_success = data.map(v => v.filter(v => !v.pay || v.pay.status !== "confirm").length);
...
const RoomBookUseTimeChartComponent = props => {
    ...
    const updateBookData = async () => {
        ...
        const newDataList = (await Promise.allSettled(dateArray.map(async v => getBooksByBookDate(v)))).map(v => v.value);
        const newDataList2 = newDataList.map((value, index) => loadBookData(value, dateArray[index]))
        ...
        newDataList2.forEach((value, index) => {
            ...
            Object.entries(value).forEach(d => {
                const [k, v] = d;
                v.filter(v => v.data.status !== 400 && v.data.pay && v.data.pay.status === "confirm").forEach(vv => {
                    const gap_hour = Number(vv.data.end_time.split(":")[0]) - Number(vv.data.start_time.split(":")[0])
                    const gap_minute = Number(vv.data.end_time.split(":")[1]) - Number(vv.data.start_time.split(":")[1])
        ...
        setChartDataSet(Object.entries(result).map((v, i) => {
...
const RoomBookUsageChartComponent = props => {
    ...
    const openToCloseArray = ([...(new Array((openToCloseTime.to - openToCloseTime.from) * 2 + 1))].map((_, i) => ((i / 2) + openToCloseTime.from)))
        .map(v => `${Math.floor(v)}:`.padStart(3, "0") + `${(v % 1) * 60}`.padStart(2, "0") )
    ...
    const updateBookData = async () => {
        ...
        const books = await getBooksByBookDate(moment(selectDate).format("YYYY-MM-DD"));
        loadBookData(books);
    }
    ...
    const filterRoomBookStartEndTime = dataList => {
        return openToCloseArray.map(v => {
            ...
            return dataList.filter(r => moment(r.startTime).isSameOrBefore(t) && moment(r.endTime).isSameOrAfter(t)).length ? 1 : 0;
        });
    }
    const chartData = Object.entries(dataShow).filter(v => v[1]).map((v, i) => {
        const key = v[0];
        if (key !== "all") {
            return {
                id: i,
            ...
            const r = [...(new Array(10))].map((_, i) => filterRoomBookStartEndTime(data["r_" + `${i + 1}`.padStart(2, "0")]));
            const d = []
...
const SnackbarPayChartComponent = props => {
    ...
    const updateSnackBarPrice = async () => {
        setData((await Promise.allSettled(dateArray.map(async v => getSnackBarPaysByDate(v)))).map(v => v.value));
    }
    ...
    return <Line
        ...
        data={{
            ...
            datasets: [{
                    ...
                    type: 'line',
                    label: "순 수익",
                    data: data.map(v => v.reduce((p, c) => p + c.total_price - (c.total_original_price + Math.floor(c.total_price / 10)), 0)),
                }]
        }}
    />
}
const SnackbarProductStockChartComponent = props => {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const getAllStocks = newSnackBarProducts.map(async v => await getSnackBarStock(v.barcode))
            const newSnackBarStocks = (await Promise.allSettled(getAllStocks)).map(v => v.value);
            ...
            const vc = newSnackBarStocks
                ...
                const quantityData = [...dateArray].reverse().map((v, index, array) => {
                    if (index === 0) {
                        return quantity
                    }
                    else if (sortedData[array[index - 1]]) {
                        quantity = quantity - _.sum(sortedData[array[index - 1]].map(d => d.quantity))
                    }
                    else {}
                    return quantity
                }).reverse();
...
const ChartPage = props => {
    ...
    return <div>
        ...
        <div style={{display: "flex", justifyContent: "center"}}>
            ...
            <div>
                <Form.Select onChange={e => SetSelectDateDuration(Number(e.target.value))}>
            ...
            <Form>
                <div className="mb-3">
                    <Form.Check type="checkbox" label={"예약 관련 정보"} onClick={e => setCheckData({...checkData, room_book: e.target.checked})} />
                    <Form.Check type="checkbox" label={"예약 시간대 별 사용 정보"} onClick={e => setCheckData({...checkData, room_book_usage: e.target.checked})} />
                </div>
            </Form>
            <Form>
                <div className="mb-3">
                    <Form.Check type="checkbox" label={"각 방별 사용 현황"} onClick={e => setCheckData({...checkData, room_book_daily_usage: e.target.checked})} />
                    {/*<Form.Check type="checkbox" label={"예약 시간대 별 사용 정보"} onClick={e => setCheckData({...checkData, room_book_usage: e.target.checked})} />*/}
                </div>
            </Form>
            <Form>
                <div className="mb-3">
                    <Form.Check type="checkbox" label={"스낵바 매출 정보"} onClick={e => setCheckData({...checkData, snackbar_pay: e.target.checked})} />
                    <Form.Check type="checkbox" label={"스낵바 상품 재고 정보"} onClick={e => setCheckData({...checkData, snackbar_product_stock: e.target.checked})} />
                </div>
            </Form>
        </div>
        <div style={{display: "flex", flexDirection: "column", marginRight: 10}}>
            {checkData.room_book ? <RoomBookChartComponent selectDate={selectDate} selectDateDuration={selectDateDuration} /> : null}
            {checkData.room_book_usage ? <RoomBookUsageChartComponent selectDate={selectDate} selectDateDuration={selectDateDuration} /> : null}
            {checkData.room_book_daily_usage ? <RoomBookUseTimeChartComponent selectDate={selectDate} selectDateDuration={selectDateDuration} /> : null}

            {checkData.snackbar_pay ? <SnackbarPayChartComponent selectDate={selectDate} selectDateDuration={selectDateDuration} /> : null}
            {checkData.snackbar_product_stock ? <SnackbarProductStockChartComponent selectDate={selectDate} selectDateDuration={selectDateDuration} /> : null}
            {checkData.snackbar_product_stock ? <SnackbarProductPayChartComponent selectDate={selectDate} selectDateDuration={selectDateDuration} /> : null}
