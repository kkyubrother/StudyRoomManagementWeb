...
// import {Button, ButtonGroup, Col, Dropdown, DropdownButton, Form, Modal, Row, Table} from "react-bootstrap";
...
import {getPay, PAY_STATUS_CONFIRM, PAY_STATUS_REJECT, PAY_STATUS_WAITING, postPay} from "../api/pays";
import axios from "axios";

const cancelBookByAdmin = (book_id, Authorization) => {
    return axios.delete(`/api/books/${book_id}/admin`, {headers:{Authorization}}).then(response => response.data)
}
...
const user = {"age":null,"birthday":null,"chat_id":-1001583498229,"created":"2022-01-31T23:54:44","delete_que":null,"department":null,"gender":null,"grade":20,"modified":"2022-01-31T23:54:44","num":null,"sms":1,"status":null,"tg_name":null,"user_id":1,"username":"웹사이트","valid":true}
const BlockerPage = () => {
    const peopleNo = -5;
    const purpose = "block";
    ...
    const handleOnSelectEndTime = (hour, minute) => {
        const t = moment(`${hour}:${minute}`, "HH:mm")
        if (startTime && t.isSameOrBefore(startTime)) alert("올바르지 않은 시간입니다.");
        else setEndTime(t);
    }
    ...
    const handlePostPay = async result => {
        ...
        for (let i = 0; i < 60; i++) {
            ...
            if (pay_result.status.startsWith(PAY_STATUS_CONFIRM)) return {result: true, message: "성공"}
    ...
    const handleStrategyStartToEnd = async room => {
        return await handlePostPay(await postBook(
            null,
            user.user_id,
            bookDate,
    ...
    const handleStrategyStartToEndIncludeExist = async (room, start_time, end_time) => {
        return await handlePostPay(await postBook(
            null,
            user.user_id,
            bookDate,
    ...
    const handleStrategyDeleteAndBlock = async (room, books) => {
        ...
        const filterTime = v => {
            ...
            const f = x => (m => m.hour() + m.minute() / 60)((t => moment(moment(t).format("HH:mm"), "HH:mm"))(x));
        ...
        const book_keys = books[room_key]
            .filter(filterTime)
            .map(book => book.id);

        await Promise
            .allSettled(book_keys.map(book_id => cancelBookByAdmin(book_id, authorization)));
    ...
    const handleOnClickBlock = _ => {
        if (!bookDate) alert("날짜 설정 필요")
        ...
        else if (Object.values(room).filter(v => v.enable).length === 0) alert("장소 설정 필요")
            ...
            const parseResult = (v, i) => {
                if (v.status === "rejected") {
                    return {result: false, room: `${rooms[i].type}_${rooms[i].no}`, message: v.reason.response.data.message}
                }
                else {
                    return {result: true, room: `${rooms[i].type}_${rooms[i].no}`, message: v.value.message}
                }
            }

            if (blockStrategy === 1) {
                Promise
                    .allSettled(rooms.map(handleStrategyStartToEnd))
                    .then(results => setBlockResult(results.map(parseResult)))
                    .then(() => setImgSrc(`/api/books/${bookDate}.png?_=${(new Date()).valueOf()}`))
                    .then(() => setIsProcessing(false));
            }
            else if (blockStrategy === 2) {
                const time_gap = Number((endTime.hour() - startTime.hour()) * 2 + (endTime.minute() - startTime.minute()) / 30);
                ...
                time_list.map(value => {
                    Promise
                        .allSettled(rooms.map(room => handleStrategyStartToEndIncludeExist(room, value.start_time, value.end_time)))
                        .then(results => setBlockResult(results.map(parseResult)))
                        .then(() => setImgSrc(`/api/books/${bookDate}.png?_=${(new Date()).valueOf()}`))
                        .then(() => setIsProcessing(false));
                })
            ...
            //    3. 1번과 2번이 참인 예약
                const loadBookData = (data) => {
                    ...
                    data.forEach((value, index) => {
                        if (!moment(value.book_date).isSame(moment(bookDate).format("YYYY-MM-DD")))
                ...
                getBooksByBookDate(moment(bookDate).format("YYYY-MM-DD"))
                    ...
                    .then(books => {
                        Promise
                            .allSettled(rooms.map(room => handleStrategyDeleteAndBlock(room, books)))
                            .then(results => setBlockResult(results.map(parseResult)))
                            .then(() => setImgSrc(`/api/books/${bookDate}.png?_=${(new Date()).valueOf()}`))
                            .then(() => setIsProcessing(false));
                    })
    ...
    const blockStrategyTitle = blockStrategy === 0
        ? "막기 전략을 선택하세요."
        : blockStrategy === 1 ? '"시작 ~ 끝" 시간이 비어있다면 막기'
            : blockStrategy === 2 ? '"시작 ~ 끝" 시간 중에서 비어있는 곳 막기'
                : blockStrategy === 3 ? '"시작 ~ 끝" 시간 중에서 겹치는 예약을 취소시키고 막기'
                    : "다시 선택하세요."
    ...
    return <div>
        ...
        <div style={styles.container}>
            ...
            >
                ...
                <InputDateV3 value={bookDate} setValue={v => handleOnSelectDate(v.getFullYear(), v.getMonth() + 1, v.getDate())} />
                <InputStartEndTimeV2
                ...
                <Form.Group as={Row} controlId="room">
                    <Form.Label column sm="2">대여 장소</Form.Label>
                    <Col sm="10">
                        <Col style={{backgroundColor: "#82e0aa"}}>
                            <span>스터디 룸: </span>
                            {
                                [...new Array(10)]
                                    .map((_, i) => (<Form.Check
                                            ...
                                            onChange={e => handleOnSelectRoom(1, i + 1, e.target.checked)}
                                        />
                            ...
                            <Form.Check
                                ...
                                name="group1"
                                type="checkbox"
                                checked={room[`1_1`] && room[`1_1`].enable && room[`1_2`] && room[`1_2`].enable && room[`1_3`] && room[`1_3`].enable && room[`1_4`] && room[`1_4`].enable && room[`1_5`] && room[`1_5`].enable && room[`1_6`] && room[`1_6`].enable && room[`1_7`] && room[`1_7`].enable && room[`1_8`] && room[`1_8`].enable && room[`1_9`] && room[`1_9`].enable && room[`1_10`] && room[`1_10`].enable}
                                onChange={e => {
                                    const newRoom = {...room};
                ...
                />
                <Form.Group as={Row}>
                    <Form.Label column sm="2">막기 전략</Form.Label>
                    <Col>
                        <DropdownButton
                            as={ButtonGroup}
                        ...
                        >
                            <Dropdown.Item eventKey={1} >"시작 ~ 끝" 시간 전체가 비어있는 것만 막기</Dropdown.Item>
                            <Dropdown.Item eventKey={2} >"시작 ~ 끝" 시간 중에서 비어있는 곳만 막기</Dropdown.Item>
                            <Dropdown.Item eventKey={3} >"시작 ~ 끝" 시간 중에서 겹치는 예약을 취소시키고 막기</Dropdown.Item>
                        </DropdownButton>
                    </Col>
                </Form.Group>
                <Button
