    ...
    getPay(pay_id)
      .then((pay_result) => {
        if (pay_result.status.startsWith(PAY_STATUS_CONFIRM)) {
          callback({
            status: PAY_STATUS_CONFIRM,
            title: "결제 완료",
            message: `결제되었습니다.`,
          });
        } else if (pay_result.status.startsWith(PAY_STATUS_REJECT)) {
          if (user_id && book_id)
            deleteBook(user_id, book_id, `Pay.${PAY_STATUS_REJECT}`).then(
              (res) => console.log(res),
            );
          callback({
            status: PAY_STATUS_REJECT,
            title: "결제 실패",
            message: pay_result.comment
              ? pay_result.comment
              : "다시 결제바랍니다.",
          });
          ...
          setTimeout(
            WaitTransaction,
            500,
            ...[pay_id, timer - 1, callback, user_id, book_id],
          );
        }
      })
      .catch((error) => {
        if (user_id && book_id)
          deleteBook(user_id, book_id, `Pay.Error`).then((res) =>
            console.log(res),
          );
        callback({
          status: PAY_STATUS_REJECT,
