...
const cancelBook = (book_id, qr_code, cashier) =>
  axios.put(
    `/api/books/${book_id}`,
    { action: "cancel", cashier: cashier },
    { headers: { Authorization: `QR ${qr_code}` } },
  );
  ...
  getPay(pay_id)
    .then((result) => {
      if (result.status === "confirm") {
        ...
        updateMessage(
          `${result.comment}(${Math.round(timer / 2)} 초 남음})`,
          true,
        );
        if (timer > 0) {
          setTimeout(waitingPay, 500, ...[pay_id, timer - 1, updateMessage]);
        } else {
...
export default function cancelBookControl(
  book_id,
  qr_code,
  cashier,
  updateMessage,
  ...
  cancelBook(book_id, qr_code, cashier)
    ...
    .catch((error) => {
      if (error.response.status === 403) {
        ...
        updateMessage(
          `[${error.response.status}]오류가 발생했습니다(${error.response.data.message})`,
          false,
        );
      }
    });
