...
const GuestBookInfoContainer = ({book, disabled, pay_options = ["card", "saved_money.d"]}) => {
    const timer = 60;
    ...
    const handleOnClickDelete = () => {
        ...
        cancelBookByAdmin(book_id, authorization)
        ...
        .then(pay => {
            ...
            const wait_transaction = (pay_id, timer) => {
                ...
                if (timer > 0) {
                    getPay(pay_id)
                        .then(pay_result => {
                            if (pay_result.status.startsWith(PAY_STATUS_CONFIRM)) {setMessage(`결제되었습니다.`);}
                            else if (pay_result.status.startsWith(PAY_STATUS_REJECT)) {setMessage(pay_result.comment?pay_result.comment:"다시 결제바랍니다.");}
                            else if (pay_result.status.startsWith(PAY_STATUS_WAITING)) {setTimeout(wait_transaction, 500, ...[pay_id, (timer - 1)])}
                        })
                        .catch(e => setMessage(e?e.response.message:"다시 결제바랍니다."))
                } else {setMessage("결제 대기 시간 초과하였습니다. 다시 결제바랍니다.");}
            }
    ...
    const handleChangeRoom = async room => {
        try {
            const response = await putBookChangeRoom(book.book_id, room.type, room.no);
    ...
    return <Container fluid>
        ...
        <InputRoomV3
            ...
            disabled={false}
        />
        {changeRoomText ? <span style={{color: "red"}}>{changeRoomText}</span> : null}
        ...
        <InputPay
            ...
            options={pay_options}
        />
        <Row><Col>{(message && (message !== "결제되었습니다." && message !== "삭제 성공")) && <Spinner animation={"border"} variant={"primary"} />}{message && message}</Col></Row>
        <Row>
            <Col sm={6}>{show_spinner && <Spinner animation={"border"} />}</Col>
            {(!message && !disabled) ? <Button as={Col} sm={2} onClick={handleOnClickPay}>결제</Button> : <Col sm={2}>{""}</Col>}
            <Col sm={2}>{""}</Col>
            <Button variant={"danger"} as={Col} sm={2} onClick={handleOnClickDelete}>삭제</Button>
        </Row>
    </Container>
}
