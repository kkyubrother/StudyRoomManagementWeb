...
export function getRooms({
  ...
  if (typeof date !== "string")
    date = `${d.getFullYear()}-${d.getMonth() > 8 ? "" : "0"}${
      d.getMonth() + 1
  ...
  if (start_time && end_time) {
    // @ts-ignore
    config.params["start_time"] =
      typeof start_time === "object"
        ? start_time.format("HH:mm:ssZ")
        : start_time;
    // @ts-ignore
    config.params["end_time"] =
      typeof end_time === "object" ? end_time.format("HH:mm:ssZ") : end_time;
  }
  // @ts-ignore
  return axios
