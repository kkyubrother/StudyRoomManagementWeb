...
export function getPay(pay_id: number): Promise<Pay> {
  // @ts-ignore
...
export function getPays(
  ...
  cashier: string | undefined = undefined,
): Promise<Pay[]> {
  // @ts-ignore
  return axios
    .get(`/api/pays`, { params: { date, pay_type, user_id, book_id, cashier } })
    .then((value) => value.data.pays);
}
export function getPaysByDate(date: string) {
  // @ts-ignore
  return axios
    .get(`/api/pays`, { params: { date } })
    .then((value) => value.data.pays);
}
...
export function getAllTransactions(): Promise<Transaction[]> {
  ...
  return axios
    ...
    .then((value) => value.data.transactions);
}
