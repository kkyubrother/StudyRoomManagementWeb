...
export function getBooksBy(user_id: number): Promise<Book[]> {
  // @ts-ignore
  return axios
    .get("/api/books", { params: { user_id: user_id } })
    .then((value) => value.data)
    .then((value) => value.books);
}
...
export function getBooksByBookDate(book_date: string): Promise<Book[]> {
  // @ts-ignore
  return axios
    .get("/api/books", { params: { book_date: book_date } })
    .then((value) => value.data)
    .then((value) => value.books);
}

export function putBookChangeRoom(
  book_id: number,
  room_type: number,
  room_no: number,
): Promise<Room> {
...
export function deleteBook(
  ...
  // @ts-ignore
  return axios
    .delete(`/api/books/${book_id}`, { data: data })
    .then((value) => value.data);
}
